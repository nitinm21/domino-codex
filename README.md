...
- **Another Domino install already provided `domino-recorder`.** That is expected. Codex now uses `domino-codex-recorder`, so the two installs can coexist without sharing a PATH entry.
